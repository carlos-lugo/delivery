...
if ($_GET['id_order']) {
	...
	$author = Restaurant::find_by_what('id', $_SESSION['user_id']);
	$body = "";	

	if (isset($_POST['body'])) {
		$body = trim($_POST['body']);

		$new_comment = Comment::make($author->id, $order->id, $body, 'Restaurante');
		if($new_comment && $new_comment->save()) {
			...
			redirect_to("userest-order-details.php?id_order={$order->id}");
		} else {
			// Failed
			$message = "Error, comment was not saved.";
		}
	}
	
} else { redirect_to('userest-order-open.php'); }
...
<div class="container">
	...
	<table class="table table-hover table-responsive table-striped">
		...
		<tr>
			<td>Order reception time</td>
			<td><?php echo $order->reception_time; ?></td>
		</tr>
		<tr>
			<td>Order requested time</td>
	...
	<h3>Order comments</h3>
		...
		?>
			<p><?php echo htmlentities($comment_user->full_name()); ?> escribi&oacute;:</p>
			<blockquote>
				<?php echo strip_tags($comment->text, '<strong><em><p>'); ?>
				<footer><?php echo datetime_to_text($comment->created); ?></footer>
			</blockquote>
		<?php endforeach; ?>
		<?php if(empty($comments)) { echo "Sin comentarios."; } ?>

	<div id="comment-form">
		<h3>Post a comment</h3>
		<form action="userest-order-details.php?id_order=<?php echo $order->id; ?>" method="POST" class="form-horizontal" role="form">
