...
<div class="container">
	...
	<form id="filter" action="bike-order-close.php" method="POST" class="form-horizontal" role="form">
		<h4>Filtro de b&uacute;squeda</h4>
		<div class="form-group">
	    	<label class="control-label col-sm-2" for="autocomplete">Reporte del:</label> 
	    	<div class="col-sm-3">
	    		<label class="radio-inline">
					<input type="radio" name="lapse" value="day" <?php if($lapse=='day') echo 'checked'; ?>>D&iacute;a
    			</label>
    			<label class="radio-inline">
					<input type="radio" name="lapse" value="month" <?php if($lapse=='month') echo 'checked'; ?>>Mes<br>
    			</label>
    		</div>
    	</div>
    	<div class="form-group">
	    	<div class="col-sm-offset-2 col-sm-10">
				...
				<select name="month" form="filter">
					...
					<option value="08" <?php if($month==8) echo 'selected'; ?>>Agust</option>
	...
	<?php
		if($orders_array) {
			...
			echo '</table></div>';
			echo '<p class="bg-primary">Total delivery cost: '.$total_cost.' Euros</p>';
			echo '<p class="bg-primary">Delivery boy earn: '.$total_bike_profit.' Euros</p><br>';
		} else {
